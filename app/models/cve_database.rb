class CveDatabase < ActiveRecord::Base
  ...
  def self.populate_data_from_csv_xml(file_path)
    @doc = Nokogiri::XML::Reader(File.open(file_path))

    cve_database_ids = CveDatabase.pluck :cve_database_id
    count = 0
    ...
    @doc.each do |node|
      ...
      next if cve_database_ids.include?(node.attribute('name'))

      puts "Adding CVE item #{node.attribute('name')}"
